...
double ort, min, max,adt;
List<double> sayilarim = new();
double[] sayilar = { 499.75, 1_999.85, 1_501.15, 999.25, 5_250, 4_750, 15_000 };
sayilarim.AddRange(sayilar);
double toplam = Topla(sayilarim, out ort,out max, out min, out adt );
WriteLine($"Toplam: {toplam:0,0.00}");
WriteLine($"Ortalama: {ort:0,0.00}");
WriteLine($"Maksimum: {max:0,0.00}");
